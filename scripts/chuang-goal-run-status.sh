...
export FORMAT WATCHDOG_REPORT_FILE RUN_ROOT OVERNIGHT_STATUS_FILE
...
def infer_overall(watchdog, overnight):
    if watchdog.get("available") and watchdog.get("readable"):
        if watchdog.get("tmux_session_present") is True and (watchdog.get("codex_process_count") or 0) > 0:
            return "terminal_worker_observed"
...
payload = {
    ...
    "watchdog": watchdog,
    "overnight": overnight,
    "overall_status": infer_overall(watchdog, overnight),
}
