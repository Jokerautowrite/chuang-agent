...
function buildReceiptCommandReply() {
  return {
    ...
    replyText: [
      ...
      "",
      "这条命令只显示 live receipt 模板入口和字段说明，不执行脚本、不读取 secret、不进入 Agent 主链。",
      "",
      "实际回执脚本：",
      "",
      ...
      "- `service_receipts`：逐项记录 `feishu`、`provider`、`subagent_live_rehearsal`、`desktop`、`browser`、`wiki`、`gbrain` 的证据引用。",
      "- `readonly_boundaries` / `service_evidence` / `real_live_acceptance`：锁定只读边界、每项证据槽位和“模板不能自动标记完成”的状态。",
