...
function buildReceiptCommandReply() {
  return {
    ...
    replyText: [
      ...
      "",
      "这条命令只显示与 `scripts/chuang-live-operator-receipt.sh --json` 同源的最新模板文案，不执行脚本、不读取 secret、不进入 Agent 主链。",
      "",
      "实际回执脚本：",
      "",
      ...
      "- `tested_at`：现场回执时间。",
      "- `request_id`：本次人工 live 检查请求编号。",
      "- `operator`：人工操作人。",
      "- `env_file`：本次检查使用的 Chuang Feishu env 文件。",
      "- `workspace_root`：Chuang 仓库根目录。",
      "- `approval_scope`：本次批准的精确 live 范围。",
      "- `rollback_condition`：中止或回退条件。",
      "- `acceptance_status`：`not_verified` / `verified` / `blocked`。",
      "- `can_mark_real_live_ready`：只有真实 operator evidence 完整后才可为 true。",
      "- `cannot_mark_complete_without_operator_evidence`：模板默认 true，不能因为生成模板就标记 live 完成。",
      ...
      "- `readonly_boundaries` / `boundaries`：锁定只读边界，两个字段同源同值。",
      "- `service_evidence`：逐项记录 `feishu`、`provider`、`subagent_live_rehearsal`、`desktop`、`browser`、`wiki`、`gbrain` 的证据引用。",
      "- `service_receipts`：逐项记录 `feishu`、`provider`、`subagent_live_rehearsal`、`desktop`、`browser`、`wiki`、`gbrain` 的证据清单。",
      "- `real_live_acceptance`：`complete=false`、`status=not_verified`、`gap_count=7`、`cannot_mark_complete_from_template=true`、`requires_operator_evidence=true`。",
      "- `codex_hermes_isolation`：保持 Codex/Hermes 隔离的简短说明。",
      "- `notes`：人工备注。",
      "- `blockers`：阻塞项列表。",
      "- `boundaries`：保密和只读边界。",
      "",
      "环境变量只用于本地写模板时补默认值：`CHUANG_LIVE_OPERATOR`、`CHUANG_AGENT_ROOT`、`CHUANG_LIVE_ENV_FILE`、`CHUANG_LIVE_OPERATOR_ENV_FILE`、`CHUANG_FEISHU_ENV_FILE`、`CHUANG_LIVE_REQUEST_ID`。",
      "",
