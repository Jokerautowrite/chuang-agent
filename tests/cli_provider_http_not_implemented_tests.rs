...
fn cli_run_http_transport_surfaces_https_scheme_error_with_request_preview() {
    ...
    assert!(
        stdout.contains("CONFIG_ERROR: openai-compatible provider invalid field=base_url reason=unsupported_http_scheme:https://api.example.com/v1/responses"),
        "stdout={stdout}"
    );
    assert!(
...
fn cli_run_http_transport_reports_invalid_port_shape() {
    ...
    assert!(
        stdout.contains("CONFIG_ERROR: openai-compatible provider invalid field=base_url reason=invalid_port:http://127.0.0.1:notaport/v1/responses"),
        "stdout={stdout}"
    );
    assert!(
