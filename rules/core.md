...
10. Full local workspace: inside the configured workspace root (see
    `permission.workspace_root` in config), normal read, write, patch, build,
    test, scan, report, screenshot, locate, app, mouse, and keyboard actions
...
17. Dispatch by complexity, parallelize maximally: choose subagent policy by task complexity (analyze for read-heavy discovery, execute for bounded implementation, orchestrate for multi-part work); when a task can be split into independent units, spawn as many parallel subagents as the system can run (max_concurrency up to the configured cap), do not serialize what can run in parallel. Simple tasks do not need a subagent; hard tasks must not be done single-threaded when parallelism helps. Prefer dispatching subagents through the OpenCode fork tooling (opencodex) when the native spawn channel is capped; keep the opencodex endpoint and model namespace configurable, never hardcoded.
