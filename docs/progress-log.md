...
- live operator receipt flow 继续增强：`scripts/chuang-live-operator-receipt.sh --json` 现在输出 `request_id`、`approval_scope`、`rollback_condition`、`readonly_boundaries`、`service_evidence`、`service_receipts` 和 `real_live_acceptance`，覆盖 Feishu/provider/single subagent rehearsal/desktop/browser/wiki/GBrain 七类 evidence 槽位；`scripts/chuang-live-operator-checklist.sh` 的 real-live acceptance matrix 也对齐为 7 项。模板仍固定 `can_mark_real_live_ready=false`，不连接真实服务、不读 secret、不启动 worker、不修改仓库。`scripts/chuang-candidate-verify.sh` 和 `scripts/chuang-third-test-smoke.sh` 已纳入 receipt 模板结构断言，`/receipt` 飞书文案和 smoke 同步更新。
