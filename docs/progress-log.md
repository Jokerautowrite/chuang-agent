...
- provider readiness 本地只读检查通过：`scripts/chuang-provider-readiness-check.sh --json` 输出 `overall_state=ready`、`provider_kind=openai_compatible`、`transport=native`、`api_key_state=<set>`、`connects_real_provider=false`、`request_timeout_ms=120000`。但同轮 Feishu 普通文本触发真实 provider 路径时返回上游 `502 Bad Gateway`，因此 provider live acceptance 当前记为 `blocked_by_upstream_502`，不能标记 verified。
...
- 当前结论：Feishu health/session/tools 现场证据已拿到，provider 配置 ready 但 live request blocked，real live 仍未 100% 完成；desktop/browser/wiki/GBrain 仍需单独 evidence。
