...
The bridge handles `/new`, `/session`, `/health`, `/receipt`, `/live-receipt`, `/live-check`, `/tools` / `/capabilities`, and `/help` locally as bridge commands. `/new` starts a fresh app-server thread and binds the current Feishu chat to it; `/session` reports that binding; `/health` reports local bridge/app-server/provider-env status with secret values shown only as `<set>` or `<missing>`; `/receipt` and `/live-receipt` show the manual live receipt template entry without running the script or reading secrets; `/live-check` shows the manual live test commands without running them or executing any local checklist; `/tools` / `/capabilities` shows the mounted local abilities and hard boundaries; `/help` lists the local commands. App-server failures during `/new` or normal `turn/start` are returned to Feishu as sanitized operational messages. These commands are not forwarded as user tasks to the Agent runtime.

`/live-check` result states are operator-facing only: `ready` means all required local checks pass with no blocker, `blocked` means a required env/workspace/config/app-server/session/read-only check must be fixed before live use, and `warning` means a non-blocking item needs human review. Do not send secrets, tokens, app secrets, API keys, or full env contents back to chat; report only `<set>/<missing>` and short sanitized errors.
