...
把 wiki / GBrain 作为外脑知识库层接入 Chuang，但只走本地只读 preview / source-contract 边界，不把外脑直接写进核心主链。
...
当前只接本地 markdown/text 根目录的只读检索和 preview，不连接真实 wiki/GBrain live retrieval，不做自动写回，不注入 runtime。
