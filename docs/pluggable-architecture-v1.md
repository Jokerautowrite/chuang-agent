...
### 0.1 解耦是为了「调动最强」，不是处处自研最强

可插拔的目的：**随时把当前最强的 agent / 后端挂上来打工**，而不是在每一个赛道上自研到第一。

- 编码工人更强 → 换/调 Codex（或同类），不在创内核里重做一套编码 IDE Agent。
- 模型更强 → 换 provider slot。
- 桌面控制更强 → 换 actuator adapter。

创守住调度、记忆、治理；工人可换、可并行、可审计。  
若某项工作业界已有明显更强的专用 agent，**默认调用它**，不要在创仓库里死磕对标。  
完整表述见 `docs/blueprint-v1.md` §0.1「调度台原则」。
