...
| Feishu live contact | 已推进 | 老爸在 Chuang 专用 Feishu 会话中确认 `/health`、`/session`、`/tools`：bridge=ready、app-server=running、session=`chuang-thread-1`、workspace=`/home/user/projects/chuang-agent`、Feishu/provider env 均为 `<set>` | 说明 bridge/session/tools 现场证据已拿到；普通文本已触发 provider 路径但返回上游 502，因此不能把 provider live 写成 verified |
| provider evidence | blocked live request | `scripts/chuang-provider-readiness-check.sh --json` -> `overall_state=ready`、`transport=native`、`api_key_state=<set>`；Feishu 普通文本返回 `502 Bad Gateway` | readiness/env 对齐已通过；真实 provider live request 当前被上游 502 阻断，必须修复或重试后才能记 verified |
