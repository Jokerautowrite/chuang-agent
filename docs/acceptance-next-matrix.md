...
| provider evidence | 已完成 | `cargo test -q --test slot_registry_tests --test runtime_report_tests`；`scripts/chuang-provider-readiness-check.sh` | fallback/capacity/retryable 诊断合同已存在；provider readiness check 已进入候选门禁；live 前仍要人工确认 env 对齐，并把真实 provider live receipt 和 readiness `<set>/<missing>` 分开记录 |
| subagent evidence | 已完成 | `cargo test -q --test cli_subagent_live_preflight_tests`；`scripts/chuang-live-runner-readiness-view.sh --json` 可聚合 live runner blocked reason | gate/allowlist/capability/report admission rehearsal 已有；readiness view 只是本地只读视图，下一步只允许单 worker live rehearsal |
